...
public class VoidAI : MonoBehaviour {
	...
	void Update () {
		...
		if(bIsVoidsTurn && !bHasWon)
		{
			if(!bHasCenter)
				...
				if(GetVoidTileCount() <= 125)
					RandomMove();//SpreadTowardsPlayer();
			else
				SpreadTowardsPlayer();
	...
	void RandomMove()
	{
		...
		while(!bHasValidTile)
		{
			foreach (GameObject tile in VoidTiles)
			{
				...
				//VoidTiles.AddRange(TmpVoidTiles);
			}
			ClearValidMoves();
			//VoidTiles.AddRange(TmpVoidTiles);
			//TmpVoidTiles.Clear ();
	...
	void SpreadTowardsPlayer()
	{
		...
		print ("-----Begin SpreadTowardsPlayer-----");
		ChosenTilePosition=PickClosestTileToPlayer().transform.position;
		minTile=null;
		print ("Closest Tile: "+ChosenTilePosition);
		
		GetValidMoves(ChosenTilePosition);
		
		/*print ("----------------"+i+"----------------");
		foreach (string shaft in ValidMoves)
			print(shaft);*/
		
		D2P=GetDistanceToPlayer(ChosenTilePosition);
		
		print("Distance to Player: "+D2P);
		
		desiredMove=DetermineQuadrant(D2P);
		
		print ("Desired move: "+desiredMove);
	...
	GameObject PickClosestTileToCenter()
	{
		...
		foreach (GameObject tile in VoidTiles)
		{
			...
			//print ("Current distance to center: "+currentDist);
			if(currentDist < currentMinDist)
			{
				currentMinDist=currentDist;
				minTile=tile; //assigned here
